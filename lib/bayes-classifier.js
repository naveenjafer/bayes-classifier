...
BayesClassifier.prototype.addDocument = function(doc, label) {
  ...
  for (var i = 0; i < doc.length; i++) {
      this.features[doc[i].toString()] = 1;
  }

  //to generate the bigrams.
  for (var i = 0; i < doc.length-1; i++) {
      this.features[doc[i] + " " + doc[i+1]] = 1;
  }

  // to generate the trigrams
  for (var i = 0; i < doc.length-2; i++) {
      this.features[doc[i] + " " + doc[i+1] + " " + doc[i+2]] = 1;
  }
 ...
 */
 function generateNgrams(n, doc)
 {
   // n is the gram size to generate.
   if(n > 0)
   {
     gram_doc = []
     for(var i = 0 ; i <= doc.length - n ; i++)
     {
       gram_text = ""
       for(var j = i ; j < i+n ; j++)
       {
         gram_text += " "
         gram_text = gram_text + doc[j]
       }
       gram_doc.push(gram_text.trim().toString())
     }
     return gram_doc;
   }
   else
...
BayesClassifier.prototype.docToFeatures = function(doc) {
  ...
  doc1 = generateNgrams(1,doc)
  doc2 = generateNgrams(2,doc)
  doc3 = generateNgrams(3,doc)
  doc = doc1.concat(doc2)
  doc = doc.concat(doc3)

  //generate all unigrams, bigrams and trigrams of doc.
...
BayesClassifier.prototype.getClassificationsAsPercent = function(doc) {
  var classifier = this;
  var labels = [];
  var total = 0
  for (var className in this.classFeatures) {
    var value_prob = classifier.probabilityOfClass(this.docToFeatures(doc), className)
    labels.push({
      label: className,
      value: value_prob
    });
  ...
  for(var label in labels){
    labels[label]["value"] = ((labels[label]["value"]/total)*100).toFixed(2)
    labels[label]["value"] = labels[label]["value"].toString() + "%"
  }
  return labels
};
/**
